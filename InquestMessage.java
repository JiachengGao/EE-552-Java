...
public class InquestMessage extends JDialog implements ActionListener{
	...
	{
		super(f,"searching",false);
		Container con=getContentPane();
		con.setLayout(new FlowLayout());
		this.file=file;
		txlnumber=new JTextField(5);
		tname=new JTextField(5);
		inquesNo=new Button("by NO.");
		inquesNo.addActionListener(this);
		inquesName=new Button("by Name");
		inquesName.addActionListener(this);
		inquesAll=new Button("search all");
		inquesAll.addActionListener(this);
		con.add(new Label("input searching NO."));
		con.add(txlnumber);
		con.add(inquesNo);
		con.add(new Label("input searching Name"));
		con.add(tname);
		con.add(inquesName);
		con.add(inquesAll);
		con.add(show);
